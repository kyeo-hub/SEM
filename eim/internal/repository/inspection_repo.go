...
// InspectionRepository 检查记录数据访问层
type InspectionRepository struct {
	db *gorm.DB
}

// NewInspectionRepository 创建检查记录仓库实例
...
// GetByEquipmentID 根据设备 ID 获取检查记录列表
...
// GetByEquipmentAndDate 根据设备 ID 和日期获取检查记录
...
// GetByEquipmentAndShift 根据设备 ID 和班次获取检查记录
...
// GetDetailsByRecordID 根据记录 ID 获取检查明细
...
// CountByDateRange 统计日期范围内的检查记录数
